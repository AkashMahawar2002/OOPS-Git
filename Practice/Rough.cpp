...
public:
    Sorter() : cap(nullptr), size(0) {}

    Sorter(int* inputData, int n) : cap(inputData), size(n) {}
...
public:
    NumberCombinerSorter(int* inputData, int n) : Sorter(inputData, n) {}

    void sort() override {
        cout << "Now subclass sorting\n";
        for (int i = 0; i < size - 1; i++) {
            for (int j = size - 1; i < j; j--) {
                if (cap[j] < cap[j - 1]) {
                    swap(cap[j], cap[j - 1]);
